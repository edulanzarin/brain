...
- **Projeto não linka projeto.** Dois sistemas que não trocam dado não se linkam, mesmo
  ...
  base. Errado: `Navedesk — Reusa o visual de: [[Questor BI]]`. Certo:
  `Navedesk — Usa: [[Design]]`. Só linke projeto a projeto se houver relação real:
...
- **Par de portas reservado** (`40xx` app / `54xx` banco, mesmo final) — consultar e
  atualizar `[[Uma faixa de portas por projeto]]`.
...
- Um projeto, uma cor, **só na nota do projeto**: questor-bi azul `3900150` ·
