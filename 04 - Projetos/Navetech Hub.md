...
### Janelas flutuantes (ago/2026)

Primeiro primitivo de **janela flutuante** do Nexo (`components/ui/janela.tsx` + store `lib/janelas.ts` + `useArrastar` + a `CamadaJanelas` montada nos Providers): o primo **não-modal** do Modal — várias coexistem, arrastam pela barra, vêm pra frente ao foco e deixam o fundo vivo, pra **consultar/comparar lado a lado** sem perder o de trás. Piloto na **Conferência**: clicar numa nota abre o detalhe (`NotaDetalheConteudo`, extraído do `NotaDetalheModal` pra servir aos dois) numa janela em vez de modal — dá pra abrir várias notas e comparar; reabrir a mesma foca a existente em vez de duplicar. As **Pendências** seguem no Modal (bloquear o fundo é a UX certa lá). O conceito e as regras (mesma casca, intenção oposta; `z` entre chrome e modal; arraste comita ao soltar) em [[Janela flutuante é o primo não-modal do modal]]. Escopo v1: arrastar/focar/fechar; minimizar/maximizar/snap e restaurar layout ficam pra v2.
