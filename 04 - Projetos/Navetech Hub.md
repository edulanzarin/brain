...
- **Caminho alternativo de import (a investigar)**: o Questor tem **Implantação de Saldos nativa** (`implsaldoctb`), que grava saldo por conta direto (natureza + valor, sem contrapartida/histórico/complemento) — ver [[Módulo contábil do Questor]]. Se ele expõe layout de importação pra isso, dispensaria a transitória e o histórico que o caminho atual (importar lançamentos) exige.
