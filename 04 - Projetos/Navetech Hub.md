...
**Dois pontos cegos do escopo fechados (jul/2026)** — auditoria pela pergunta "nenhuma função acessa empresa fora do escopo?". (a) Um monte de rota de **empresa avulsa** do Contábil (`plano`, `conferencia`, balancetes, `bf-check`, `aprender`, `contas`, `extrato-*`) e o `nota-itens` (Fiscal/Contábil) liam `?empresa=` cru e consultavam o Questor sem re-checar o escopo — só tinham o gate de módulo. Novo `assertEmpresaVisivel` (em `api-route.ts`) barra empresa fora do alcance da sessão, aplicado em toda rota de empresa única. (b) As empresas do **RH** (NAVECON/FOUR/FINAVE, `EMPRESAS_RH`) vazavam para os outros módulos e para quem via "todas": `podeVerEmpresa` passou a recusá-las fora do RH e o fragmento `condForaDoRh` as exclui nos funis e no seletor `/api/empresas`; o RH segue com escopo forçado próprio, intacto. Lição em [[Drill-down por id foge do funil de escopo e precisa de gate próprio]].
