...
A seção virou **"Balancete"** com **duas abas do mesmo trabalho** (mesmo filtro empresa+mês): **Balancete** (botão "Gerar") monta a **balancete de verificação contábil** de verdade — por conta do plano, saldo anterior · débito · crédito do período · saldo atual, na árvore do `classifconta` com corte por nível, igual o Questor imprime (`/contabil/balancete-contabil`, `resumirBalanceteVerificacao` sobre o mesmo `coletarBalanceteContabil`, que fecha ao centavo: Σ débito = Σ crédito, Σ saldo devedor = Σ saldo credor); e **Análise** (botão "Analisar") roda o motor/laudo sobre ela. Trocar de aba mantém o balancete carregado (cache por empresa+período). Mostra só contas com saldo ou movimento (as zeradas já saem no coletor). O `id` da seção segue "analise" (chave de permissão), só o rótulo/caminho mudaram.
