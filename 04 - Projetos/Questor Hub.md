...
- **NFSE S/ Retenção × C/ Retenção — só uma o motor reproduz**: o CFOP de serviço tomado decide a fórmula do débito. **S/ Retenção → `vlrContISS`** (token que o motor não tem → pula → cai em "extra"/"Verificar manual"). **C/ Retenção → `vlrContabil`** (o motor TEM → reproduz → dá pra pegar anomalia). Ex.: NFSE 9144 (488, CFOP 8001004 Software C/Ret) o motor reproduziu e pegou conta errada — plano manda D 4533 (Sistemas), contábil lançou D 4942. Ou seja, nem toda NFSE é "verificar manual" — só as S/Ret.
- **Por que não dá pra "só reproduzir NFSE S/Ret" (tentado e revertido, jul/2026)**: a S/Ret usa `vlrContISS`, token que o motor não fornece → pula. Adicionar `vlrContISS = valorcontabil` (no tomador o ISS não é destacado, então é o cheio) reproduziu a NFSE, MAS **super-produziu R$ 7,8 MILHÕES de falsa diferença** numa conta (488, Comissões): uma vez que o token existe, o motor gera a despesa para TODA nota cujo CFOP mapeia ali via o config/aprendido (ruidoso p/ serviço), inclusive as que no real não foram pra aquela conta — e o `observadas` deixa passar porque a conta recebe lançamento por nota. É exatamente a incompletude que o "espelho honesto" evita: **o motor não sabe atribuir nota de serviço à conta certa**. Reproduzir NFSE de verdade exigiria ISS real + atribuição confiável por nota, que não existe limpa pra serviço. Decisão: seguir espelhando NFSE e rotular.
