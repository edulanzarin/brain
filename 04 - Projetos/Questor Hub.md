...
- Escopo restante: a completude 100% do motor (imposto/retenção nota a nota) segue grind aberto — a decisão é que o valor real é **detecção de anomalia** (Auditoria), não reprodução perfeita. Contas com muitas notas (>2000 linhas) truncam a lista do drill (soma parcial, sinalizada).
