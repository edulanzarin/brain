...
- **Aba Diferenças** (jul/2026): não é seção nova — é a **mesma comparação**, então virou aba do Balancete (decisão: o que faz parte da mesma coisa não vira menu à parte). Lista as contas analíticas com diferença e, ao clicar, mostra **quais notas** a causam: por nota, o líquido (débito−crédito) esperado × o real na conta. A soma fecha com a coluna Diferença (reconciliação exata). Cada nota rotulada: "Valor diferente" (anomalia forte), "Não lançada aqui" (esperada, foi p/ outra conta), "Sem regra reproduzível". Reusa o motor via um modo `net` do `DetalheFiscal`. Dá pra clicar na diferença tanto na aba quanto no próprio balancete (mesmo modal, compartilhado). Os modais de nota mostram a **espécie** (NFE/NFSE); a NFSE que o motor não reproduz vem marcada **"Verificar manual"** (âmbar), deixando explícito que serviço se confere à mão.
...
- **O motor SUPER-produz (o outro lado da moeda) — diferença exatamente N× é sinal**: quando o esperado é um múltiplo redondo do real (2×, 3×), quase sempre é o motor, não o contábil. Dois padrões achados (jul/2026):
  ...
  - **CFOP com apura de ST ligada, nota sem ST (pendente)**: o CFOP 2101002 (1015) tem `apurasubtribut='1'` e a tabela de ST é IDÊNTICA à de mercadoria (as duas `D 3089 = vlrContICMS`). O motor dispara o componente de ST pelo flag do CFOP mesmo a nota não tendo ST → posta 3089 duas vezes (481,60 → 963,20). O contábil lança uma. Fix exige gatear os componentes de imposto pelo VALOR do imposto na nota, não só pelo flag `apura*` do CFOP — grind aberto.
- **Por que não dá pra "só reproduzir NFSE" (tentado e revertido, jul/2026)**: a NFSE não é reproduzida porque a fórmula do plano é `vlrContISS` (valor contábil líquido de ISS), token que o motor não fornece → pula. Adicionar `vlrContISS = valorcontabil` (no tomador o ISS não é destacado, então é o cheio) reproduziu a NFSE, MAS **super-produziu R$ 7,8 MILHÕES de falsa diferença** numa conta (488, Comissões): uma vez que o token existe, o motor gera a despesa para TODA nota cujo CFOP mapeia ali via o config/aprendido (ruidoso p/ serviço), inclusive as que no real não foram pra aquela conta — e o `observadas` deixa passar porque a conta recebe lançamento por nota. É exatamente a incompletude que o "espelho honesto" evita: **o motor não sabe atribuir nota de serviço à conta certa**. Reproduzir NFSE de verdade exigiria ISS real + atribuição confiável por nota, que não existe limpa pra serviço. Decisão: seguir espelhando NFSE e rotular.
