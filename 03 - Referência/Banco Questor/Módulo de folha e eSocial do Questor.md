...
- `funcescala` (jornada/horário — a vigência liga o contrato à `escala`), `funcctps`, `funclegais`, `funcsindicato`, `funcadicional` (insalubridade/periculosidade), etc. **`escala` é cadastro GLOBAL** (PK só `codigoescala`, sem `codigoempresa`); `descrescala` é o **horário legível** (ex.: "08:00 às 12:00/14:00 às 18:00"), com `turno` e `cargahorsemanal`. A view `funcionario` já traz `codigoescala` e `descrtipojornada` (a descrição da jornada do `funcescala`, resolvida pela vigência). **Para quebrar/filtrar por "horário", use `descrtipojornada` com `escala.descrescala` de reserva** — `coalesce(nullif(btrim(descrtipojornada),''), nullif(btrim(descrescala),''), '(sem horário)')`. Motivo: `descrtipojornada` é campo livre e é ali que o DP escreve o TURNO ("1º turno", "2º turno"), enquanto `descrescala` é o horário cru, cada empresa com uma grafia ("13:30 às 17:30/18:00 às 22:00", "07:30 12:00", "05:00 AS 09:00 DAS 09:30 AS 13:00 SABADO..."), o que faz cada empresa virar um grupo só dela. Corrigido em set/2026, a pedido do próprio DP — ver [[O agrupamento útil sai do campo que o operador preenche]].
...
Conta-se por **contrato**, não por pessoa (uma pessoa com dois vínculos conta dois). Armadilha: **transferência** entre estabelecimentos da mesma empresa gera par desligamento+admissão que infla o índice. Recorte de vínculo: nesta base o `codigocateg` (smallint) vem **nulo** — quem tem valor é `funccontrato.categoria` (varchar, ex.: `'01'` empregado, `'07'`, `'11'`) e `tipovinculo` (varchar, ex.: `'10'` CLT, `'80'` diretor); use esses pra filtrar CLT. E `codigosit` é status cadastral (=1 pra todos), **não** serve de ativo/inativo — o vivo/desligado é `datadem`. Mas `datadem` nulo só diz que ninguém registrou a demissão: em set/2026, 3,3 mil de 8 mil contratos CLT sem demissão não tinham folha havia 120 dias. Ativo de verdade é ter folha recente ([[Contrato sem demissão não prova funcionário ativo no Questor]]). A série mensal sai do padrão [[Estoque e fluxo numa série a partir de datas de início e fim]].
...
- **Rescisões calculadas** → `rescisao`: `codigocausa`, `dataavprevio`, `dataresc`, `codigopercalculo`. `datahoralcto` é o carimbo do **último cálculo**. O `codigopercalculo` liga à folha de rescisão em `periodocalculo` (`codigotipocalc = 60`), de onde sai `datapgto` — que é a data **PREVISTA** de pagamento gravada no cálculo, **não** o pagamento efetivo (some antes do dinheiro sair). Serve de sinal de apoio ("já calculada, pgto previsto para tal dia"), nunca de prova de quitação — ver [[Uma pendência de prazo fecha por ato explícito, não por sinal inferido]]. Mesma armadilha do `complementar` dos avisos: nesta base a rescisão vem com `complementar = 1`; pegue a de MENOR `complementar` por contrato (`order by complementar limit 1`), não fixe `= 0`.
...
- **Férias calculadas** → `reciboferias` (PK inclui `datainicial` = início do **período aquisitivo**): `datainicialferias`/`datafinalferias` (gozo), `datapgto`.
  - **Controle de férias vencidas / a vencer**: NÃO há (no schema conhecido) tabela de saldo de período aquisitivo em aberto — **deriva-se** de `dataadm` (períodos de 12 em 12 meses) cruzado com os aquisitivos já gozados em `reciboferias.datainicial`. Regra CLT: aquisitivo completo (12m) dá 30 dias; a empresa tem +12m para conceder (concessivo), senão paga em **dobro** → limite = fim do aquisitivo + 12m; passou sem recibo = **vencida**. Aproximação gerencial: ignora afastamento (suspende/zera o aquisitivo), falta (reduz/perde direito) e parcial/coletiva. Dois cortes obrigatórios (set/2026): só contrato com folha recente, e só período cujo limite de concessão cai depois da primeira folha do contrato no Questor; sem eles, as vencidas do escritório saíam 1.469 em vez de 126 ([[Contrato sem demissão não prova funcionário ativo no Questor]]). A **tabela nativa de saldo do Questor fica a mapear** (o cadastro de férias tem período aquisitivo com dias de direito/gozo/saldo — achar o nome quando houver acesso). Visto em: [[Navetech Hub]] (Folha → Férias).
...
A **rescisão** liga pelo mesmo caminho no evento **S-2299** (tabela `esocialdadoss2299`, mesmo padrão do 2200 — a confirmar na base). Para um **panorama de conformidade** (sem ligar ao contrato): agrupe `esocialtransacao` por `evento` no período (`datahoralcto::date`) e classifique cada linha — `recibo` preenchido = **aceito**, sem recibo e `status = 13` = **rejeitado**, o resto = **pendente**. Conte só `evento like 'S-%'`: a tabela guarda também o envelope do lote e a EFD-Reinf ([[esocialtransacao guarda lote e EFD-Reinf junto dos eventos do eSocial]]). Dá o volume transmitido × resultado por tipo de evento. Visto em: [[Navetech Hub]] (Folha → eSocial).
