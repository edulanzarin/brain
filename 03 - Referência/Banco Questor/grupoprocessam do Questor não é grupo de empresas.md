...
Ao montar o [[Questor Hub]], a intuição foi usar `grupoprocessam` (tem `descrgrupoproc` com nomes tipo "Empresas Larissa") + `grupoempresa` (vínculo `codigogrupoproc`↔`codigoempresaproc`) como "grupos de empresas" pra filtrar. **Errado**: esses grupos existem pra rotinas internas do Questor (processamento em lote, cálculo de folha etc.), não pra análise. Os nomes enganam.
