...
No [[Questor Hub]] (Next.js 16, App Router), os filtros escreviam o estado na URL via `router.replace(\`?\${params}\`)`. Em `next dev` funcionava; em `next build && next start` o clique não alterava a URL nem refazia as queries — falha **silenciosa**, sem erro no console.
